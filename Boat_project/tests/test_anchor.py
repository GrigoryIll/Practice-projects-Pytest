...
@pytest.mark.parametrize("boat_is_afloat, anchor_in_boat, boat_is_anchored, expected_result", [(True, True, False, False), (False, True, False, True), 
                                                                                               (False, True, True, True), (False, False, False, True), 
                                                                                               (False, False, True, True)],
                                                                    ids = ["valid1", "invalid1", "invalid2", "invalid3", "invalid4"])
def test_boat_anchor_func(boat_is_afloat, anchor_in_boat, boat_is_anchored, expected_result, prepare_boat):
    ...
    if boat_carolina.anchor and boat_carolina.is_afloat and not boat_carolina.is_anchored:
        ...
        with pytest.raises(ValueError) as excinfo:
            boat_carolina.set_anchor()
        assert f"Лодка не на воде, либо якорь уже установлен/якоря нет" in str(excinfo.value)
