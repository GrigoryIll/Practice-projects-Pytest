...
class Rowlock:
    ...
    def detach(self, boat: Boat):
        ...
        if self.is_attached == False:
            raise ValueError(f"{self.hand} уключина уже демонтирована")
        elif self.hand in boat.oars:
            raise ValueError(f"Сначало нужно вытащить весло из {self.hand} уключины")
    ...
    def check_functionality(self, status: bool):
        ...
        if status not in STATUS:
            ...
            self.functionality = True
            print(f"Функционально")
        else:
            self.functionality = False
            print(f"Нефункционально")