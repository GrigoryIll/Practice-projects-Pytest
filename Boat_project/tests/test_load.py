...
@pytest.mark.parametrize("total_boat_weight, water_in_boat_status, expected_weight, expected_water_in_boat", 
                                                                                               [(300, False, 300, False), (299, False, 299, False), 
                                                                                               (301, True, 301, True), (300, "Yes", 300, False)],
                                                                                               ids = ["valid1", "valid2", "invalid1", "invalid2"])
def test_boat_max_load(total_boat_weight, water_in_boat_status, expected_weight, expected_water_in_boat, prepare_boat):
    ...
    if boat_carolina.max_weight < expected_weight:
        ...
        assert (f"Превышен допустимый максимальный вес лодки в {boat_carolina.max_weight} кг") in str(excinfo.value)
    else:
        for weight in weight_list:
            luke.to_aboard(weight, boat_carolina)
    ...
    if water_in_boat_status not in boat_carolina.water_in_boat_status:
        ...
        assert (f"Вода либо: {boat_carolina.water_in_boat_status}") in str(excinfo.value)
    else:
        boat_carolina.check_water_in_boat(water_in_boat_status)
