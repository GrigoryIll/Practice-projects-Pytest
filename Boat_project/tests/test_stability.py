...
@pytest.mark.parametrize("seat, status_capsized, expected_result",
                         [("нос", False, False), ("корма", False, False),
...
def test_boat_stability(seat, status_capsized, expected_result, prepare_boat):
    ...
    if seat not in boat_carolina.seats:
        ...
        assert f"Пересесть невозможно. Доступны места: {boat_carolina.seats}" in str(
            excinfo.value)
    elif status_capsized not in boat_carolina.capsized_status:
        with pytest.raises(ValueError) as excinfo:
            boat_carolina.check_capsized(status_capsized)
        assert f"Лодка либо: {boat_carolina.boat_availability_status}" in str(
            excinfo.value)
    else:
        luke.change_seat(seat, boat_carolina)
        boat_carolina.check_capsized(status_capsized)
