...
class Passenger:
    ...
    def change_seat(self, seat: str, boat: Boat):
        ...
        if seat not in boat.seats:
            raise ValueError(
                f"Пересесть невозможно. Доступны места: {boat.seats}")
    ...
    def to_aboard(self, item: Union[Weight, Equipment], boat: Boat):
        ...
        print(
            f"На борт принят груз {item.weight} кг., вес лодки {boat.current_weight} кг")
        if boat.current_weight > boat.max_weight:
            raise ValueError(
                f"Превышен допустимый максимальный вес лодки в {boat.max_weight} кг")
    ...
    def to_disaboard(self, item: Union[Weight, Equipment], boat: Boat):
        ...
        boat.current_weight -= item.weight
        print(
            f"C борта снят груз {item.weight} кг, вес лодки {boat.current_weight} кг")
